...
class SitzungsFaden(QThread):
    ...
    # zustand, ansage, detail, pfad, taetigkeit, ablehnung, hinweis
    ereignis_da = Signal(str, str, str, str, str, bool, bool)
    text_da = Signal(str)
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.ereignis_da.emit(
                "fehler", "Arbeitsfaden abgestürzt", str(fehler), "", "", False, False
            )
        finally:
            ...
            if art == "modell":
                try:
                    ...
                    self.ereignis_da.emit(
                        ...
                        str(fehler), "", "", False, False
                    )
                continue
            try:
                self.ereignis_da.emit("denkt", "Auftrag laeuft", text[:120], "", "", False, False)
